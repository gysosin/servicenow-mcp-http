...
The ServiceNow MCP server can expose the Streamable HTTP transport so MCP clients can connect using the standard `/mcp` endpoint.

#### Starting the HTTP Server

You can start the HTTP server using the provided CLI:
...
The server listens on `/mcp` by default:

- `POST /mcp` - send JSON-RPC messages (set `Accept: application/json, text/event-stream` and `Content-Type: application/json`)
- `GET /mcp` - open the event stream for responses

You can change the base path by setting the `SERVICENOW_HTTP_BASE_PATH` environment variable (for example, `SERVICENOW_HTTP_BASE_PATH=/api/mcp`).

#### Example

See the `examples/sse_server_example.py` file for a complete example of setting up and running the HTTP server.
...
# Obtain the low-level MCP server and build the HTTP app (defaults to /mcp)
